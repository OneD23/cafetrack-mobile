...
function formatCategory(categoryId: string) {
  return categoryId.replace('cat-', '').replace('-', ' ').replace(/\b\w/g, (char) => char.toUpperCase());
}
    ...
    <TouchableOpacity onPress={onPress} activeOpacity={0.9}>
      <Card style={{ marginBottom: theme.spacing.md, padding: 0, overflow: 'hidden' }}>
        ...
        <View style={{ padding: theme.spacing.lg }}>
          ...
          <Text style={{ marginTop: 4, color: theme.colors.textSecondary }}>{formatCategory(business.categoryId)}</Text>
